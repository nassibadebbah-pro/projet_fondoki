...
if (isset($_POST['submit'])) {
    ...
    $Tchambr = 'Triple Room'; // Pure English Database Value

    // Query to find an available room of type 'Triple Room' with precise calendar overlap checking
    $sql = "SELECT nbrCH FROM chambre WHERE type='Triple Room' AND nbrCH NOT IN (
                SELECT DISTINCT nbrCH FROM reservations WHERE (datR < ? AND datS > ?)
            ) LIMIT 1";
            
    $stmt = $conn->prepare($sql);
    // Bind Check-Out date first, then Check-In date
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Secure insert operation using SQL prepared statement injection guard
        ...
        $message = "Inventory Error: No unbooked triple occupancy spaces match selected timelines.";
        $message_type = "error";
    }
    $stmt->close();
}
...
<head>
    ...
    <meta name="description" content="Triple occupancy booking engine tracking configuration deck.">
    <link rel="stylesheet" href="/css/style.css"/>
    <link rel="icon" href="/icons/logo.png">
    <link href="https://fonts.googleapis.com/css2?family=Tangerine&display=swap" rel="stylesheet"/>
   
    <title>Triple Occupancy Accommodations Control Panel</title>
</head>
<body dir="ltr">
...
<div style="max-width: 1200px; margin: 30px auto; padding: 20px; font-family: sans-serif;">
    ...
    <section class="table-container">
        ...
        <h2 style="margin-top: 20px; color:#18181b;">Create New Triple Room Reservation</h2>
        ...
        <h2 style="color:#18181b;">Currently Reserved Triple Rooms</h2>
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 40px; background: #fff; border: 1px solid #e4e4e7;">
            ...
            <tbody>
                <?php
                $date_today = date("Y-m-d");
                $sql_reserved = "SELECT * FROM reservations WHERE Tchambr='Triple Room' AND datS >= ? ORDER BY datR ASC";
                ...
                if($res_bookings->num_rows > 0) {
                    while($row = mysqli_fetch_assoc($res_bookings)){
                        ...
                        echo '<tr style="border-bottom: 1px solid #f4f4f5;">
                                ...
                                <td style="padding: 12px;">Triple Room</td>
                    ...
                    echo '<tr><td colspan="7" style="text-align: center; padding: 20px; color:#a1a1aa;">No active triple room reservations recorded on today\'s schedule ledger.</td></tr>';
                }
                $stmt_res->close();
                ?>
            </tbody>
        ...
        <h2 style="color:#18181b;">Available Triple Rooms Inventory</h2>
        <table style="width: 100%; border-collapse: collapse; background: #fff; border: 1px solid #e4e4e7;">
            ...
            <tbody>
                <?php
                $sql_available = "SELECT * FROM chambre WHERE type='Triple Room' AND nbrCH NOT IN (
                                    SELECT DISTINCT nbrCH FROM reservations WHERE (datR < ? AND datS > ?)
                                  ) ORDER BY nbrCH ASC";
                ...
                if($res_avail->num_rows > 0) {
                    ...
                    echo '<tr><td colspan="3" style="text-align: center; padding: 20px; color:#a1a1aa;">100% Occupancy reached. No triple room spaces currently vacant.</td></tr>';
                }
                $stmt_avail->close();
                ?>
            </tbody>
